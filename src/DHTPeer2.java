...
public class DHTPeer2 implements Runnable {
    ...
    public DHTPeer2(int peerPort, String portType) {
        this.peerPort = peerPort;
        this.portType = portType;
    }

    public static void main(String[] args) {
        Thread leftPeer = new Thread(new DHTPeer2(2821, "left"));
        Thread rightPeer = new Thread(new DHTPeer2(2822, "right"));
        Thread queryPeer = new Thread(new DHTPeer2(2823, "query"));
            ...
            while (true) {
                ...
                if (command.startsWith("query")) {
                    ...
                    if (peerId == identifier) {
                        ...
                        DatagramPacket peer2Response = new DatagramPacket(buffer, buffer.length,
                                request.getAddress(), request.getPort());
                        socket.send(peer2Response);
                    }
                }
            }
    ...
    public void setupPortLeft(int peerLeftPort) {
        System.out.println("Left 2 Started");
        try {
...
//                            InetAddress.getLocalHost(), 2812);
                        ...
                        request = new DatagramPacket(packet, packet.length, InetAddress.getLocalHost(), 2822);
                        socket.send(request);
                    }
...
//            c++;
    ...
    private void setupPortRight(int peerPort) {
        System.out.println("Right 2 Started");
        try {
